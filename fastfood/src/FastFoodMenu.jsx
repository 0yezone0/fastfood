      ...
      <div className="flex-1 p-4">
          ...
          >
            {categories.map((cat, idx) => (
              <div
                ...
                className={`min-w-[200px] max-w-[200px] h-[140px] bg-white rounded-xl shadow-md flex flex-col items-center justify-center cursor-pointer transition-transform ${
                  selectedCategory === cat.name ? 'bg-orange-300 scale-105' : 'hover:scale-105'
                }`}
              >
        ...
        <div className="grid grid-cols-3 gap-4">
          {filteredProducts.map((product, index) => (
            <div
              key={product.id}
              className="bg-white p-4 rounded-lg shadow text-center flex flex-col justify-between transform transition duration-300 ease-in-out hover:scale-105 hover:shadow-xl cursor-pointer"
              onClick={() => openProductDetail(product)} // mở chi tiết khi click toàn khối
            >
              ...
              {/* nút mua không bị click lan lên cha nhờ stopPropagation */}
              <button
